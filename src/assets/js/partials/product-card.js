...
class ProductCard extends HTMLElement {
  ...
  getCardTemplate() {
    return `
      ...
      <a href="${this.product?.url}" class="block group">
        ...
        ${this.product?.subtitle && !this.minimal ? `<div class="text-center text-gray-400 text-sm mb-2">${this.product?.subtitle}</div>` : ''}
  ...
  attachEventListeners() {
    this.querySelectorAll('[name="donating_amount"]').forEach((element) => {
      element.addEventListener('input', (e) => {
        e.target.closest('.s-product-card-content')
  ...
  initCircleBar() {
    ...
    const total = qty > 100 ? qty * 2 : 100;
    const roundPercent = (qty / total) * 100;
    const bar = this.querySelector('.s-product-card-content-pie-svg-bar');
  ...
  getProductPrice() {
    ...
    return `<span class="text-primary text-base font-bold">${this.getPriceFormat(this.product?.price)}</span>`;
  }
